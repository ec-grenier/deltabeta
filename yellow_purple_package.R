...
library(ncdf4)
library(rlist)
#library(narray) should avoid to load this because it masks import packages like base::subset.....
library(tidyr)
library(cowplot)
library(gdata)
library(gtable)
...
#' makes a csvv draw. Can return same format as read.csvv(). coeffs and sigma should have the format returned by read.csvv()
#' @param coeffs data frame, with three named elements : 'prednames', 'covarnames', 'gamma'. Same format returned by read.csvv().
#' @param sigma numerical matrix, same format returned by read.csvv().
#' @param n integer, number of draws to make.
#' @param seed seed for the rng
#' @param matrix_decomposition one of 'eigen', 'svd', 'chol'
#' @param return_format currently only 'read.csvv.data.table' supported. Returns 'csvv' data is if you had called read.csvv(), but in a data table format, expanded `n` times (`n` draws) 
#' @param add_means 
#' @return all the draws in a format depending on return_format. 
rmvnorm.csvv <- function(coeffs, sigma, n=1, seed=1, matrix_decomposition='eigen', return_format='read.csvv.data.table', add_means=TRUE){

	prednames <- coeffs$prednames
	covarnames <- coeffs$covarnames


	estimates <- list(mu=coeffs$gamma, sigma=sigma)
    set.seed(seed)
	muhat <- rmvnorm(n=n, mean=estimates$mu, sigma=estimates$sigma, method=matrix_decomposition)
	dimnames(muhat) <- list(1:nrow(muhat), paste0(prednames,' X ',covarnames))

	if(add_means){
		muhat <- rbind(muhat, matrix(estimates$mu, ncol=length(estimates$mu), nrow=1))	
	}

	if (grepl('read.csvv',return_format)){
		if(return_format=='read.csvv.data.table'){
			muhat <- rbindlist(lapply(1:nrow(muhat), function(i) data.table(i=i, prednames=prednames, covarnames=covarnames,gamma=muhat[i,])))
		}
	}
...
#' produces a mortality curve and clips it depending on the clipping type requested. Only clipping parameters are documented. 
#' @param minpoly_constraint integer. upper bound defining the range of temperatures within which to find the curve minimum. the lower bound is always 10C.  
#' @param do.clipping logical. if TRUE, 'levels-clipping' is performed. 'levels clipping' means that we rule out strictly negative values -- any value below 0 is set to 0. That is, mortality can't decrease, relative to the MMT. 
#' @param goodmoney.clipping logical. if TRUE, so-called 'goodmoney' clipping is performed. 
#' It means that we compare the fully-adapted and the only-climate-adapted curves, and we take for each temperature point the minimum of the two. This means that we do not allow 
#' income to increase the mortality sensitivity, but to only decrease it or not affect it. Hence, 'money' must be 'good' ('goodmoney'). 
#' @param do.diffclip logical. if TRUE, 'U-clipping' is performed. 'U-clipping' refers to 'weak monotonicity assumption'. It means that we force the curve to be weakly monotonic away from the calculated
#' optimum : it can't decrease and the increase, it can either start by decreasing, continuously or to become flat (hence 'weakly'), or vice versa, start by decreasing, continuously up to the end or become flat. 
get.clipped.curve.mortality <- function(csvv, climtas, loggdppc, age, year, region, adapt='full', base_year=2010, TT_lower_bound=-23, TT_upper_bound=45, TT_step = 1, model="poly", minpoly_constraint=30, do.clipping=T, goodmoney.clipping=T, do.diffclip=T, get.covariates=F, covars=NULL, covar.names=NULL, list.names=NULL, CI='central', ...) {
	TT=round(seq(TT_lower_bound, TT_upper_bound, TT_step/2),3)
	...
	if (model == "poly") {         
		if (do.clipping) { #levels-clipping
			...
			if (CI == 'upper' | CI == 'lower') {
			  ...
			  yy0 <- yy0 - yy0[ii.min]
				...
				yyg <- yyg - yyg[ii.min]
				yyi <- get.curve.mortality(TT=TT, csvv=csvv, climtas=climtas[1], loggdppc=loggdppc[2], age=age) #income adapt only (No climate adapt)
				yyi <- yyi - yyi[ii.min]
			...
			# clipping happening here. pmax/pmin are vectorized min/max functions. 
			if (goodmoney.clipping) {
				yy_goodmoney <- pmin(yy1, yyg) # good money clipping 
				yy <- pmax(yy_goodmoney, 0) # levels clipping
			} else {
				yy <- pmax(yy1, 0) # only levels clipping (no good money)
			}

			# same, but when we use the income only adapted curve. 
			if (adapt == 'income') {
				yy_goodmoney <- pmin(yyi,yy0) # good money clipping
				yy <- pmax(yy_goodmoney, 0) # levels clipping 
			} else if (adapt == 'no') {
				yy <-  pmax(yy0, 0) # only levels clipping for the no adapted curve. 
			} else if (adapt != 'full') {
			...
			if (goodmoney.clipping) {
				yy <- pmin(yy1, yyg) # Good money clipping only, no levels clipping 
			} else {
		...
		if (do.diffclip) { #u-clipping
			yy.nd <- yy
			yy[ii.min:length(yy)] <- cummax(yy[ii.min:length(yy)]) #forces values to be equal to 'max value to date' (cumulative max value). next values can't be strictly lower than previous values. 
			# that is, say optima is 20C, then 23C can't be better than 22C and 24C can't be better than 23C. 
			#yy[1:(ii.min-1)] <- rev(cummax(rev(yy[1:(ii.min-1)])))
			yy[1:(ii.min)] <- rev(cummax(rev(yy[1:(ii.min)])))
		}
    
		return(format.curve(curve=yy, year=year, base_year=base_year, region=region, TT=TT, adapt=adapt)) #EDIT HERE 
	}
}
...
format.curve <- function(curve, year, base_year, region, TT, adapt = 'full', new_tag = NULL, ...){
	if (adapt == 'full') {
		...
		year_dim = paste0(toString(year),'_clim')
...
#' @param user_values FALSE or vector of named covariate values, consistent with `covar.names`. If not FALSE, the covariates data defined by the file in `cov.dir` is ignored. 
#' @return a 'region' by 'year' data frame, with as many columns as there are covariate names. 
load.covariates <- function(regions, years, cov.dir, covarkey, covar.names, filetype='csv', subset_list=NULL, skip_lines_cov=0, user_values=FALSE, ...) {
	
	if (isFALSE(user_values)){
		#memo.csv = addMemoization(data.table::fread)
		if (filetype=='csv') {
			covars = data.table::fread(cov.dir, stringsAsFactors=F, skip=skip_lines_cov)
	  ...
	  covars = covars[,c(covarkey,'year', covar.names), with=FALSE] #covars[,c(covarkey,'year',covar.names)]
		...
		stopifnot(is.numeric(user_values) & names(user_values)==covar.names)
		message('User passed manual covariate values -- building fake set of covariate values and plugging passed region and year. Building fake covariates data.')
		DT <- as.data.table(as.list(user_values))
		stopifnot(ncol(DT)==length(covar.names) & names(DT)==covar.names)
		DT[,region:=regions][,year:=years][]
		region.year <- DT #to match the name of what's returned.
...
extract_climate_data <- function(year, gcm='CCSM4', tas_value='tas', climpath = NULL, tas.path=NULL, is.gcm=T, ncname = '1.1', rcp='rcp85', ...){
	if (is.gcm == T) {
		...
		if (is.null(tas.path)){
			tas.path = '/project/cil/gcp/climate/BCSD/hierid/popwt/daily/'
		}
		#open netcdf
...
extract_monthly_edd <- function(year, gcm='CCSM4', tas_value='edd_monthly', climpath = NULL, tas.path=NULL, is.gcm=T, ncname = '1.3', rcp='rcp85', ...){
	...
	if (is.gcm == T) {
		...
		if (is.null(tas.path)){
			tas.path = '/project/cil/gcp/climate/BCSD/hierid/cropwt/monthly/'
		}
...
bin.clim <- function(clim, clim_next=NULL, region, year, TT_lower_bound=-55, TT_upper_bound=55, TT_step = 1, daily_seasonal_climate = '',crop='', season='', binnedstuff=TRUE, ...){
	if (daily_seasonal_climate == 'edd') {
		if (crop == "wheat-winter") {
			...
			if (gs[1] > gs[2]) {	
				...
				clim = narray::stack(clim, clim_next, along=1)
		...
		TT=round(seq(TT_lower_bound, TT_upper_bound, TT_step),3)
		...
		TT=round(seq(TT_lower_bound, TT_upper_bound, TT_step),2)
		h = hist(clim[,region,toString(year)], breaks = TT, include.lowest=TRUE, plot=FALSE)
		ds = array(h$counts,dim=c(length(h$counts),1,1),dimnames=list(h$mids,year,region))
		return(ds) 
...
plot_hist <- function(region, binclim, h.tempcol=1, h.valuecol=3, h.yearcol=2, x.lim=c(-10,45), 
	hist.breaks = seq(0, 50, by = 10), hist.y.lim=c(0,50), hist.y.lab = "Number of days", 
...
plot_curve = function(region, curve_ds,  y.lim=c(-5,200), c.tempcol=1, c.valuecol=4, c.yearcol=2, x.lim=c(-10,45), colors=rev(c("#faafaf","#ff6961","#81176D","#FBC17D")), c.margin=c(.25,.25,0,.25),bounds=NULL, y.lab="Change in deaths / 100,000", ...) {
	kwargs = list(...)
	if (typeof(curve_ds)=='list') {
		curve_ds = curve_ds[[region]]
	}
	df = reshape2::melt(curve_ds[,,region, drop=F])
	...
	if (!is.null(bounds)) {
		dflist = list()
		for (yr in names(bounds[[region]])) { 
		  dflist[[yr]] = df %>% 
		    # dplyr::filter(temp > floor((bounds[[region]][[yr]][1])) & 
		    #               temp < ceiling((bounds[[region]][[yr]][2])) & 
		    #               year == yr) 
	...
	ggplot(data=df, aes(x = as.numeric(df[,c.tempcol]), y = df[,'value'], group = df[,c.yearcol])) +
		geom_line(aes(colour=factor(df[,c.yearcol])), linewidth = 1.5) +
		geom_hline(yintercept=0, linewidth=.2) + #zeroline
...
delta_beta_fa_na_only <- function(region, curve_ds, binclim, year, base_year, rnd.digits = 2, drop_zero_bins = T, rel.20 = T, bin=NULL, ...) {
	dims = dimnames(binclim)[[1]]
	...
	under20 = round( apply(deltabeta[which(as.numeric(rownames(deltabeta)) < 20),(ncol(deltabeta)-3):ncol(deltabeta)],2,sum), digits = 2)
	over20 = round( apply(deltabeta[which(as.numeric(rownames(deltabeta)) > 20),(ncol(deltabeta)-3):ncol(deltabeta)],2,sum), digits = 2)
	total = round( apply(deltabeta[,(ncol(deltabeta)-3):ncol(deltabeta)],2,sum), digits = 2)
...
delta_beta <- function(region, year, base_year, curve_ds, binclim, yp, export.path, db.style=standard_delta_beta,covars, covarkey, add.cov.table=F, return.db = F, db.suffix='', db.prefix='deltabeta_',  db.height=10, db.width=16, append.results=F,weather=NULL, csv=FALSE, device="pdf", relative_positions=NULL, ...){
	...
	for (ii in seq(1,length(split_db))) {
		if (add.cov.table==T & append.results==T) {
			if(is.null(relative_positions)) relative_positions <- list(cov_impacts=c(1,1),db_cov=c(1,1),db_yp=c(3/5, 2/5))
			impacts = get_impacts(region, year, base_year, return.table = T, ...)
			ct = covar.table(covars, region, covarkey)
			ct = plot_grid(ct, impacts, nrow = 1, rel_widths = relative_positions[['cov_impacts']])
			ct = plot_grid(split_db[[ii]], ct, nrow = 2, rel_heights = relative_positions[['db_cov']])
			pp = plot_grid(ct,yp[[region]], nrow = 1, rel_widths = relative_positions[['db_yp']])			
		} else if (add.cov.table==T & append.results==F) {
			if(is.null(relative_positions)) relative_positions <- list(db_cov=c(1,1),db_yp=c(3/5, 2/5))
			ct = covar.table(covars, region, covarkey)
			ct = plot_grid(split_db[[ii]], ct, nrow=2, rel_heights = relative_positions[['db_cov']])
			pp = plot_grid(ct,yp[[region]], nrow = 1, rel_widths = relative_positions[['db_yp']])
		} else if (add.cov.table==F & append.results==T) {
			if(is.null(relative_positions)) relative_positions <- list(db_impacts=c(1,1),db_yp=c(3/5, 2/5))
			impacts = get_impacts(region, year, base_year, return.table = T, ...)
			ct = plot_grid(split_db[[ii]], impacts, nrow=2, rel_heights = relative_positions[['db_impacts']])
			pp = plot_grid(ct,yp[[region]], nrow = 1, rel_widths = relative_positions[['db_yp']])
		} else {
			if(is.null(relative_positions)) relative_positions <- list(db_yp=c(3/5, 2/5))
			pp = plot_grid(split_db[[ii]],yp[[region]], nrow = 1, rel_widths = relative_positions[['db_yp']])
		}
	...
	if (return.db == T) {
		...
		ggsave(paste0(export.path,db.prefix,region,'-',year,db.suffix,".",device), plot=plot, width=db.width, height=db.height, units='in')
...
get_impacts <- function(region, year, impacts.dir, impacts.name, impacts.var, return.table=F, add_incadapt=T, font_scale=0.7,col_scale=2.1,...) {
	...
	ifelse(add_incadapt, suflist <- c('fulladapt','incadapt','noadapt','histclim'), suflist <- c('fulladapt','noadapt','histclim'))

	for (scn in suflist){

		suf <- nc_adapt_to_suf(scn)
		impacts[[scn]] = nc_to_DT(nc_file=paste0(impacts.dir,impacts.name,suf,'.nc4'), impact_var=impacts.var, to.data.frame=TRUE, print.nc=FALSE) %>% 
			...
			group_by(value) %>%
			dplyr:::summarize(scn = mean(impacts)) %>%
			data.frame() %>%
			spread('value','scn') %>%
...
generate_yellow_purple_ag_tmin <- function(regions, years, csvv.name, csvv.dir, base_year=2010,  bounds=NULL, inc.adapt=F, load.covariates=T, delta.beta=F, crop="", season="growing_season", ncversion_tmin, randomize_csvv=FALSE,...) {
	kwargs = list(...)
	all_years = sort(c(years, base_year))
	if (!is.null(csvv.name)){
	    message('loading csvv...')
	    csvv = read.csvv(filepath = paste0(csvv.dir,csvv.name), ...)
		if (!isFALSE(randomize_csvv)){
			csvv = as.data.frame(rmvnorm.csvv(coeffs=read.csvv(paste0(csvv.dir,csvv.name)), sigma=read.csvv(paste0(csvv.dir,csvv.name), 'gammavcv'),n=1, matrix_decomposition="svd", seed=randomize_csvv, add_means=FALSE))
		}
	} else { csvv = NULL }
	if (season != "summer" & crop == "wheat-winter") {
		gs <- mapply_gs_identifier(regions=regions,...)
		regions_to_remove <- c()
	...
	if (crop == "wheat-winter"){
		...
		clim = mapply_extract_climate_data(years=all_years, ncname=ncversion_tmin, ...) #daily minimum temperature X REGIONS X DAYS X YEARS
		clim_next = mapply_extract_climate_data(years=all_years+1, ncname=ncversion_tmin, ...)
	}
	clim = clim[,regions,,drop=F]
	clim_next = clim_next[,regions,,drop=F]
	message('binning climate data...')
	binclim = mapply_tmin_collapse(regions=regions, years=all_years, clim=clim, clim_next=clim_next, crop=crop, bin=FALSE,...)
	...
	if (delta.beta==T) {
		message('delta-beta...')
		ag_delta_beta_wrapper(regions=regions, years=years, csvv = csvv, covars = covars, base_year=base_year, yp=yp, crop=crop, season=season, ncversion=ncversion_tmin,...)
	}
	return(yp)
}


generate_yellow_purple_ag_edd <- function(regions, years, csvv.name, csvv.dir, base_year=2010, bounds=NULL, inc.adapt=F, load.covariates=T, delta.beta=F, interacted_weather=FALSE,weather=NULL,p.bins=NULL, tbar.adapt=FALSE, crop="", season="growing_season", ncversion_edd, ncversion_tasmax, randomize_csvv=FALSE,...) {
	kwargs = list(...)
	all_years = sort(c(years, base_year))
	if (season != "summer" & crop == "wheat-winter") {
		gs <- mapply_gs_identifier(regions=regions, ...)
		...
		message("---printing regions after removing regions that don't have enough months")
		print(regions)
		if (length(regions) == 0) { stop("THERE IS NO REGION IN THE LIST THAT HAS ENOUGH MONTHS FOR THE GIVEN SEASON TO PRODUCE YELLOW PURPLES.")}
	}
	...
	curve_plots = mapply_plot_curve(regions=regions,curve_ds=curve_ds, bounds=bounds, years=years, base_year=base_year, ...)
	message('loading climate data for the histogram (tasmax daily data, not monthly edd data)...')
	if (crop == "wheat-winter") {
		...
		clim = mapply_extract_climate_data(years=all_years, tas_value = 'tasmax', ncname=ncversion_tasmax, ...)
		clim_next = mapply_extract_climate_data(years=all_years+1, tas_value = 'tasmax', ncname=ncversion_tasmax,...)
	}
	...
	if (delta.beta==T) {
		message('delta-beta...')
		ag_delta_beta_wrapper(regions=regions, years=years, csvv = csvv, covars = covars, base_year=base_year, yp=yp, interacted_weather=interacted_weather,weather=weather,tbar.adapt=tbar.adapt, crop=crop, season=season, ncversion=ncversion_edd,...)
...
generate_yellow_purple_ag_precip <- function(regions, years, csvv.name, csvv.dir, p.bins=NULL, export.path=NULL, db.suffix=NULL, base_year=2010, bounds=NULL, inc.adapt=F, tbar.adapt=F, load.covariates=T, delta.beta=F, db.height=10, db.width=16, interacted_weather=FALSE, weather=NULL,crop="",season="growing_season", tas_value='pr', ncversion_precip, randomize_csvv=FALSE,...) {
	...
	message('loading climate data...')
	if (crop == "wheat-winter"){
		precip = mapply_extract_climate_data(years=all_years, climate_func=extract_ready_weather_data, crop=crop, season = season, weather.name = "monthbinpr",tas_value=tas_value, ...)
		precip_next = mapply_extract_climate_data(years=all_years+1,climate_func=extract_ready_weather_data, crop=crop, season=season, weather.name = "monthbinpr",tas_value=tas_value, ...)
	}else{
		precip = mapply_extract_climate_data(years=all_years, tas_value = tas_value, climate_func=extract_monthly_climate_data, ncname=ncversion_precip, ...)
		precip_next = mapply_extract_climate_data(years=all_years+1, tas_value = tas_value, climate_func=extract_monthly_climate_data, ncname=ncversion_precip, ...)
	}
		...
		if (crop != "wheat-winter") {
			...
			if (length(bb.ind[bb.ind %in% multiyear]) > 0) {
				...
				if (length(bb.y1) > 0) {
					...
					allprecip = narray::stack(clim_y1, clim_y2, along=1 )
				} else {
					allprecip = clim_y2
				}
			} else {
...
ag_yp_delta_beta_precip <- function(region, allclim, curve_ds_all,all_years,years, base_year,bounds,delta.beta,covars,p.bins,db.suffix,
	db.height, db.width,export.path, weather,device="pdf",...) {
	...
	if (delta.beta==T) {
		...
		ggsave(paste0(export.path,region,'-',paste(years, sep='-'),'_precip',db.suffix,".",device), plot=plot, width=db.width, height=db.height, units='in')
	}
...
ag_delta_beta_wrapper <- function(regions, years, csvv, covars, base_year,  yp,interacted_weather=FALSE, weather, tbar.adapt, daily_seasonal_climate = 'edd', crop, season, ncversion,...) {
	...
	if (daily_seasonal_climate == 'tasmin') {
		...
		all_years = sort(c(years, base_year))
		message('---loading tasmin data to be applied to the curve')
		if (crop == "wheat-winter"){
			clim = mapply_extract_climate_data(years=all_years, climate_func=extract_ready_weather_data, crop=crop, season=season, weather.name="seasonaltasmin", ...)
			clim_next = mapply_extract_climate_data(years=all_years+1,climate_func=extract_ready_weather_data, crop=crop, season=season, weather.name="seasonaltasmin", ...)
		} else {
			clim = mapply_extract_climate_data(years=all_years, ncname=ncversion, ...)
			clim_next = mapply_extract_climate_data(years=all_years+1, ncname=ncversion, ...)
		}
		clim = clim[,regions,,drop=F]
		clim_next = clim_next[,regions,,drop=F]
		kwargs = list(...)
		...
		message('---loading degree days data to be applied to the curve')
		pred.vars = unique(as.character(csvv$prednames))
		kinks = c()
		for (dd in c('gdd','kdd')) {
			if (grepl("wheat",crop)) {
		...
		if (crop == "wheat-winter"){
			...
			clim = mapply_extract_climate_data(years=all_years, climate_func=extract_monthly_edd, ncname=ncversion, ...)
			clim_next = mapply_extract_climate_data(years=all_years+1, climate_func=extract_monthly_edd, ncname=ncversion, ...)
...
#' @param gs_file character : full path to the file containing growing season info
...
get_growing_season <- function(region, gs_file, time_scale='monthly', ...) {

	df = fread(gs_file)

	if(!(all(c('plant_month', 'harvest_month', 'plant_date', 'harvest_date') %in% names(df)))) stop('incompatible growing season information dataset')
...
process_growing_season <- function(region, ...){
	...
	if (gs[1] > gs[2]) {
		...
		harv = gs[2] - 12
		ind = c(seq(gs[1],12), seq(1, harv))
		multiyear = seq(1, harv)
	} else {
		ind = seq(gs[1], gs[2])
		multiyear = NULL
	}
...
mapply_tmin_collapse <- function(regions,years,crop, bin=TRUE,...){
	#### Before subsetting regions, make a function that drops regions with not enough months of gs. 
	
	vect = expand.grid(regions=regions, years=years)
	kwargs = list(...)
	dslist = mapply(FUN=tmin.collapse, region=paste(vect[,'regions']), year=vect[,'years'], crop=crop, MoreArgs=c(kwargs, list(bin=bin)), SIMPLIFY=FALSE)
	ds = narray::stack(dslist, along=1)
...
tmin.collapse <- function(region, year, bin=TRUE, TT_lower_bound=-100, TT_upper_bound=250, TT_step = 1, clim, clim_next, crop, ...){
	...
	if (crop != "wheat-winter"){	
		...
		if(!is.null(multiyear)){
			...
			allclims <-  narray::stack(clims_average_gs, along=3)
...
edd.collapse <- function(region, year, kinks, clim, clim_next, crop, season=NULL, ...) {
	...
	if (crop != "wheat-winter") {
		if (!is.null(multiyear)){
			...
			alledds = narray::stack(edds_y1, edds_y2, along=1)
...
plot_ts = function(region, ds,  y.lim, base_period, years, t.valuecol=3, t.yearcol=1,  colors=c('#e41a1c','#4daf4a','#984ea3','#ff7f00','#ffff33','#a65628','#f781bf'), t.margin=c(0.2,0.2,0.2,0.2), y.lab="Number of Deaths", location.dict=NULL, ...) {
	...
	plot = ggplot() +
		geom_hline(data=df_scn, aes(yintercept=value, colour=as.factor(year)), linewidth = 1.0 , linetype = "dashed") +
		geom_line(data=df_base, aes(x = year, y = value), colour='#377eb8', linewidth = 1.0) +
		geom_hline(yintercept=0, linewidth=.5) + #zeroline
		theme_minimal() + 
		ylab(y.lab) +
		xlab("Baseline Year") +
		# coord_cartesian(ylim = y.lim )  + #xlim = x.lim
